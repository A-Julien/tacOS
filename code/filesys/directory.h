...
#define FileNameMaxLen        15    // for simplicity, we assume file names are <= 9 characters long
...
public:
    ...
    int Find(const char *name);        // Find the sector number of the FileHeader for file: "name"
...
private:
    ...
    int FindIndex(const char *name);    // Find the index into the directory table corresponding to "name"
};
